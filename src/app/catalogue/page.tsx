...
export default async function CataloguePage({
  ...
  const [categories, { produits, totalPages, page }] = await Promise.all([
    getCategories(),
    getProduits({ categorie: categorieActive, page: pageDemandee }),
  ]);
    ...
    <div className="flex flex-col md:flex-row gap-8">
      ...
      <aside className="w-full md:w-1/5 shrink-0">
        <div className="bg-[var(--card)] rounded-[var(--radius)] shadow-[var(--shadow)] p-4 md:sticky md:top-24">
          <h2 className="text-lg font-semibold mb-4 text-[var(--accent-dark)]">
            Catégories
          </h2>
          ...
          <ul className="space-y-1">
            <li>
              <Link
                href="/catalogue"
                className={`block px-3 py-2 rounded-[var(--radius)] transition ${
                  !categorieActive
      ...
      <section className="flex-1 min-w-0">
        <h1 className="text-3xl font-bold mb-6 text-[var(--accent-dark)]">
          {categorieActive ?? "Tous les produits"}
        </h1>
        ...
        {produits.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
            {produits.map((produit) => (
              <div
                key={produit.id}
                className="bg-[var(--card)] rounded-[var(--radius)] shadow-[var(--shadow)] overflow-hidden flex flex-col"
              >
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  <p className="text-[var(--accent-dark)] font-bold mb-3">
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-6 mt-10">
            <Link
              href={construireHref(categorieActive, page - 1)}
              className={`text-2xl w-11 h-11 flex items-center justify-center rounded-full transition ${
                page <= 1
                  ? "opacity-30 pointer-events-none"
                  : "hover:bg-gray-200 text-[var(--accent-dark)]"
              }`}
