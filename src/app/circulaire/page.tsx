...
export default async function CirculairePage({
  searchParams,
}: {
  ...
  const categorieActive =
    typeof rawCategorie === "string" ? rawCategorie : undefined;

  const [categories, { produits }] = await Promise.all([
    getCategories(),
    getProduits({ categorie: categorieActive, page: 1 }),
  ]);

  const visible = categorieActive
    ? produits.filter(
        (p) =>
          p.categorie?.trim().toLowerCase() ===
          categorieActive.trim().toLowerCase()
      )
    : produits;
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      ...
      <section className="relative border-b border-[var(--accent-dark)] px-6 py-16 sm:py-20">
        <div
          aria-hidden
          className="pointer-events-none absolute -right-24 -top-24 h-72 w-72 rounded-full bg-[var(--accent)] blur-3xl"
        />
        ...
        <div className="relative mx-auto max-w-5xl">
          ...
          <p className="mt-4 max-w-2xl text-base opacity-85 sm:text-lg">
            Découvrez les meilleurs prix sur nos produits en ligne.
          </p>
      ...
      <nav className="sticky top-0 z-10 border-b border-[var(--accent-dark)] bg-[var(--background)] px-6 py-4 backdrop-blur">
        <div className="mx-auto flex max-w-5xl flex-wrap gap-2">
          ...
          {categories.map((cat) => {
            const isActive =
              categorieActive?.toLowerCase() === cat.toLowerCase();

            return (
              <Link
                key={cat}
                href={`/circulaire?categorie=${encodeURIComponent(cat)}`}
                scroll={false}
                className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                  isActive
                    ? "bg-[var(--accent)] text-[var(--background)]"
      ...
      <section className="mx-auto max-w-5xl px-6 py-12">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {visible.map((p) => {
            ...
            const prixInitial =
              pAny.prixRegulier ??
              (pourcentageRabais < 100
                ? nouveauPrix / (1 - pourcentageRabais / 100)
                : nouveauPrix + 5);
            ...
            return (
              <article
                key={p.id}
                className="flex flex-col overflow-hidden rounded-2xl border border-[var(--accent-dark)] bg-[var(--decor-surface)] shadow-sm transition hover:shadow-md"
              >
                {/* HEADER */}
                ...
                <div className="flex-1 px-5">
                  ...
                  <p className="mt-1 text-sm opacity-80">
                    {p.description ?? "Produit en promotion"}
                  </p>
                </div>
                ...
                <div className="mt-4 flex items-end justify-between px-5 pb-5">
                  <div className="flex flex-col gap-0.5">
                    <div className="flex items-baseline gap-2">
                      <span className="text-xs font-medium line-through opacity-60">
                        {formatPrice(prixInitial)}
                      </span>
                      {economie > 0 && (
                  ...
                  {pourcentageRabais > 0 && (
                    <span className="inline-flex items-center rounded-lg bg-[var(--accent)] px-2.5 py-1 text-xs font-black tracking-wide text-[var(--background)] shadow-sm">
                      -{Math.round(pourcentageRabais)} %
                    </span>
                  )}
                </div>
              </article>
      ...
      <section className="border-t border-[var(--accent-dark)] bg-[var(--decor-surface)] px-6 py-10">
        <div className="mx-auto max-w-5xl text-center">
          <p className="text-sm opacity-80">
            Prix en vigueur selon disponibilité.
          </p>
          <Link
            href="/catalogue"
