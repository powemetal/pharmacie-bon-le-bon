  ...
  if (items.length === 0) {
    return (
      <div className="bg-[var(--card)] p-8 rounded-[var(--radius)] shadow-[var(--shadow)] text-center">
        ...
        <Link
          href="/catalogue"
          className="inline-block bg-[var(--accent)] text-white px-6 py-2 rounded-[var(--radius)] hover:bg-[var(--accent-dark)] transition font-medium"
        >
    ...
    <div className={`space-y-6 ${isPending ? "opacity-60 pointer-events-none transition-opacity" : ""}`}>
      ...
      <div className="flex justify-between items-center">
        <span className="text-gray-600">
          {items.reduce((sum, item) => sum + item.quantite, 0)} article(s) dans le panier
        </span>
        <button
          onClick={handleViderPanier}
          className="text-red-600 hover:text-red-800 text-sm font-semibold underline transition cursor-pointer"
        >
          Vider mon panier
        </button>
      </div>
              ...
              return (
                <tr key={item.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="p-4 flex items-center gap-3">
                    ...
                    <span className="font-medium text-[var(--foreground)] line-clamp-2">
                      {item.produit.nom}
                    </span>
                  </td>
                  ...
                  <td className="p-4 text-center">
                    <div className="inline-flex items-center border border-gray-300 rounded-[var(--radius)] overflow-hidden">
                      <button
                        ...
                        className="px-2.5 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold transition"
                        title="Réduire"
                      >
                        -
                      </button>
                  ...
                  <td className="p-4 text-right font-bold text-[var(--accent-dark)]">
                    {prixTotalProduit.toFixed(2)} $
                  </td>
      ...
      <div className="flex flex-col md:flex-row justify-end">
        <div className="w-full md:w-96 bg-[var(--card)] p-6 rounded-[var(--radius)] shadow-[var(--shadow)] space-y-3">
          ...
          <button
            ...
            className="w-full mt-4 py-3 bg-[var(--accent)] text-white font-bold rounded-[var(--radius)] hover:bg-[var(--accent-dark)] active:scale-[0.98] transition shadow-md"
          >
            Régler ma commande
          </button>
        </div>
