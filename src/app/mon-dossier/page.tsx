...
export default async function MonDossierPage() {
  ...
  return (
    <main className="pt-24 px-16 b600:px-4" style={{ color: "var(--foreground)" }}>
      <section className="max-w-5xl mx-auto">
        ...
        >
          <h2 className="text-2xl font-semibold mb-4" style={{ color: "var(--accent-dark)" }}>
            Informations personnelles
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <p>
              <strong>Prénom :</strong> {user.profil?.prenom}
            </p>
            <p>
              <strong>Nom :</strong> {user.profil?.nom}
            </p>
            <p>
              <strong>Téléphone :</strong> {user.profil?.telephone}
            </p>
            <p>
              <strong>Adresse :</strong> {user.profil?.adresse}
            </p>
            <p>
              <strong>Courriel :</strong> {user.courriel}
            </p>
          </div>
        </div>
        ...
        <div className="mb-10">
          ...
          {user.commandes.length === 0 && <p className="opacity-70">Aucune commande pour le moment.</p>}
            ...
            {user.commandes.map((cmd) => (
              ...
              >
                <div className="mb-4 pb-3 border-b" style={{ borderColor: "var(--accent)" }}>
                  <p className="font-semibold text-lg" style={{ color: "var(--accent-dark)" }}>
                    Commande #{cmd.id}
                  </p>
                  <p className="opacity-70 text-sm">Total : {cmd.total.toFixed(2)}$</p>
                </div>
        ...
        >
          ...
          {user.ordonnances.length === 0 && <p className="opacity-70">Aucune ordonnance soumise.</p>}
          ...
          {user.ordonnances.map((o) => (
            <div key={o.id} className="border-b pb-4 mb-4" style={{ borderColor: "var(--accent)" }}>
              ...
              <a href={o.fichierUrl} target="_blank" style={{ color: "var(--accent-dark)", textDecoration: "underline" }}>
                Voir le fichier →
              </a>
            </div>
          ))}
